function RHO = CCAF2(n, FF, epoch, IDX_best, trial, template_Synth, trial_harm, erp2use_cca, chan2use, filters)
...
for TT = 1:n.trials_block
    ...
    for HH = 1:n.harmonics
        ...
        tmp = filtfilt(                         filters.Xnotch,         filters.Ynotch, tmp ); % notch
        tmp =                       filtfilt(   filters.Xl_erp{HH,FF},  filters.Yl_erp{HH,FF}, tmp ); % Apply the Butterworth filter
        trialuse2(:,HH) = squeeze(  filtfilt(   filters.Xh_erp{HH,FF},  filters.Yh_erp{HH,FF}, tmp ) ); % Apply the Butterworth filter
