...
direct.engine = 'C:\Users\phineas\Desktop\BCI.speller.reboot\Engine3\'; % direct.engine
...
scheme = { 'cobalt.prf' 'darksteel.prf' 'matrix.prf' 'oblivion.prf' 'solarized-light.prf' 'vibrant.prf' 'darkmate.prf' 'default.prf' 'monokai.prf' 'solarized-dark.prf' 'tango.prf' 'david.prf' 'david2.prf' 'david3.prf' };
schemer_import( [ direct.schemer '\schemes\' scheme{14} ] );
...
for PP = 1:3
    ax(PP) = subplot(3,1,PP); cla; hold on
    
    if PP < 3
        plot(time, DATA(:,PP));
        scatter( time( latency ), DATA( latency, PP ) )
        scatter( time( latency(type==20) ), DATA( latency(type==20), PP), 'g' )
    else
        plot(time, [NaN; diff(DATA(:,PP))] )
    end
