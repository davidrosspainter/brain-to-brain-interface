...
for WORD = 1:n.words
    ...
    data{WORD,D.wordGen} = data{WORD,D.wordStim};
    
    for LL = 1: length(data{WORD,D.wordStim})
        CHAR = find(strcmp(characters, data{WORD,D.wordGen}(LL)));
        data{WORD,D.wordGen_IDX} = [data{WORD,D.wordGen_IDX} CHAR];
    end
    ...
    while SPELL
        ...
        switch phase
            ...
            case 'Keyboard'
                ...
                %% draw word
                cgdrawsprite(sprite.words(WORD), 0, 0 )
            ...
            case 'BCI'
                ...
                data{WORD,D.missionSuccess} = 0;
                 ...
                 if options.photodiode
                    word2type = [data{WORD,D.wordGen_IDX} 28 ];
                 end
                ...
                for LETTER = 1:data{WORD,D.maxBCIChars}
                    ...
                    for FRAME = 1:f.cue
                        ...
                        if FRAME == n.photo_frames %% read buffer to get letter
                            ...
                            [RHO, maxR, EEG, photo, prevSample, sampleIDX] = CCAF2_realtime2(n, epoch, template_Synth, erp2use_cca, Xnotch, Ynotch, Xlowpass, Ylowpass, Xharm, Yharm, prevSample, lim_epoch, cfg, blocksize, overlap, options, chan2use);
...
%                             results.sampleIDX(:,letter_count) = sampleIDX;
                            ...
                            if any(InpBCI==20) % deal with Backspaces
...
%                     if options.photodiode
                        if length(data{WORD,D.BCIWord_IDX}) >= data{WORD,D.wordGen_CharCount}
...
%     if elapsed(WORD) >= options.testtime_phase2
%        break; 
%     end
...
for WORD = 1:n.words
    lettersCued = [lettersCued      data{WORD, D.wordGen_IDX}];
    lettersTyped = [lettersTyped    data{WORD, D.BCIWord_IDX}];
end

accuracy = (sum((lettersCued(1:length(lettersTyped)) - lettersTyped) == 0)/length(lettersTyped))*100;
