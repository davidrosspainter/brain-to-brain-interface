...
if Eyelink('IsConnected') ~= ELdefaults.notconnected
    if Eyelink('CheckRecording') == 0 % i.e. if it is currently recording data
        ...
        filename_edf_displayPC1 = [ direct.data filename_edf_eyetrackPC ]; % where the edf data will be transferred to on the display PC
        filename_edf_displayPC2 = [ direct.data strrep( FNAME, '.', '_'), '.edf']; % where the edf data will be transferred to on the display PC
