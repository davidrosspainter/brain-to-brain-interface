...
%           cgdrawsprite(sprite.display,0,0) % output display
    ...
    for FRAME = 1:f.cue
        ...
        %cgdrawsprite(sprite.display, 0, 0) % output display
        cgdrawsprite(sprite.textBox, 0, textbox.offset.bci )
        cgdrawsprite( cue_box + 2000, 0, textbox.offset.bci ) % character
        ...
        for BOX = 1:n.boxes
            ...
            if BOX == cue_box
                cgdrawsprite( arrow_cue.sprite,  X(BOX), Y(BOX) ) % arrow cue
            end
    ...
    for FRAME = 1:f.flicker
        ...
        for BOX = 1:n.boxes
            ...
            % cgrect( X(BOX), Y(BOX), box.size, box.size, [y3(FRAME,BOX) y3(FRAME,BOX) y3(FRAME,BOX)] ) % box
            % cgdrawsprite( placeholder.sprite, X(BOX), Y(BOX) ) % placeholder
        ...
        if ismember( FRAME, 1:n.photo_frames )
            ...
            if options.photodiode
               ...
               cgdrawsprite( y4(FRAME,randomCueBox), mon.res(1)/2 - box.size/4, -mon.res(2)/2 + box.size/4, box.size/2, box.size/2 ) % box sprite
            end
        end
        
        fliptime.flicker(FRAME,TRIAL) = cgflip( color.background );
