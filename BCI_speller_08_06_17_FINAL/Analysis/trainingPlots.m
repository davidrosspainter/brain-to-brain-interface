function trainingPlots( direct, observer, amp_plot, n, n2, chan2use, IDX_best, f2, Hz, IDX_Hz2, template_Synth, erp2use_cca, t, RHO, theta, maxR, accuracy )

direct.results = [direct.resultsRoot '\trainingPlots\']; mkdir(direct.results);
...
TIT = [ 'fft.spike.' observer.fname.train ];
title( TIT )
...
TIT = [ 'fft.image.' observer.fname.train ];
title( TIT )
...
for HH = 1:n.harmonics
    ...
    saveas(h, [ direct.results TIT observer.fname.train '.png' ] )
...
TIT = ['CCA.Correlations.' observer.fname.train ];
suptitle(TIT)
saveas(h, [direct.results TIT '.png'])
...
TIT = ['accuracy.'  observer.fname.train '.Acc=' num2str(accuracy*100) '%'];
title(TIT)
